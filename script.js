...
const triggerBeat = (e) => {
  const { keyCode } = e;
  if (!(keyCode in beats)) return;
  const sound = beats[e.keyCode].beat;
  const button = beats[e.keyCode].button;
  button.deselect();
  button.select();
  sound.play();
};
const handleKeyClick = (e) => {
  const keyCode = e.target.getAttribute("id");
  if (!(keyCode in beats)) return;
  const sound = beats[keyCode].beat;
  const button = beats[keyCode].button;
  button.deselect();
  button.select();
  sound.play();
};
...
const toggleBoxshadow = () => {
  pauseButton.classList.add("buttonclicked");
  for (const key in beats) {
    beats[key].beat.pause();
  }
  setTimeout(() => {
    pauseButton.classList.remove("buttonclicked");
  }, 250);
};
const pauseButton = document.querySelector(".pause");
pauseButton.addEventListener("click", toggleBoxshadow);
